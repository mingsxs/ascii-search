...
Utility detects data automatically, if number with `A-F` or `a-f` is given, data will be taken as `Hex` data, otherwise, it wil be taken as `Decimal` data, you can also add prefix `0x`/`0X` or suffix `h`/`H` to mark it as a `Hex` format data.   
